...
public class CustomerController {
    ...
    User customer = userService.findByEmail(authentication.getName()).orElseThrow();
    // Convert Booking to BookingRequest (assuming a constructor or builder exists)
    BookingRequest request = new BookingRequest();
    request.setPickupLocation(booking.getPickupLocation());
    request.setDeliveryLocation(booking.getDeliveryLocation());
    request.setLoadWeight(booking.getLoadWeight());
    request.setScheduledDate(booking.getScheduledDate());
    request.setNotes(booking.getNotes());
    Booking savedBooking = bookingService.createBooking(customer, request);
